...
public class ReadShardExample {
  ...
  public static void readShardExample(HStreamClient client, String streamName, long shardId) {
    StreamShardReaderReceiver receiver =
        records -> {
          System.out.println("read " + records.getRecordId());
          if (records.getRecord().isRawRecord()) {
            System.out.println(
                "read raw record: " + Arrays.toString(records.getRecord().getRawRecord()));
          } else {
    ...
    var reader =
        client
            ...
            // Set shardId
            .shardId(shardId)
            // Optional: Set start offset, can be EARLIEST, LATEST, some specified timestamp or a
            // RecordId
            .from(new StreamShardOffset(StreamShardOffset.SpecialOffset.EARLIEST))
            // Optional: Set end offset, can be EARLIEST, LATEST, some specified timestamp or a
            // RecordId
